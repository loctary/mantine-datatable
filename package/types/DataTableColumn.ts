...
export type DataTableColumn<T = Record<string, unknown>> = {
  ...
  aggFunc?: (values: T[keyof T][]) => unknown;
   ...
   * When true, rows will be grouped by this column's value.
   * Multiple columns can have rowGroup enabled for nested grouping.
   */
  rowGroup?: boolean;
...
export type DataTableGroupColumn<T = Record<string, unknown>> = Omit<DataTableColumn<T>, 'accessor'> &
  Partial<Pick<DataTableColumn<T>, 'accessor'>> & {
    /**
